...
+---------------------------------------------------------+
 ...
 Description:
    ...
    Standard approach is 'strand specific' but the
    strand-aspecific fashion is also available
    (--strand_aspecific).
    ...
    (--IS_method 'classic', 'gauss', 'skewedG', 'dynamic)
    and each is tunable through specific arguments
...
###Parsing Arguments############################################################################################################################################################
usage_example = '''\n\nExample of usage for 'classic' IS-retrieving-method: python Integration_Analysis.py (--host 172.25.39.57) (--user readonly) (--pw readonlypswd) (--port 3306) --dbDataset "sequence_mld01.fu18m,sequence_mld02.fu18m" (--query_steps 5000000) (--reference_genome hg19) --columns sample,tissue,treatment (--columnsToGroup sample) --IS_method classic (--bp_rule 3) --strand_aspecific (--collision (--set_radius 4)) (--tsv) (--seqTracker)
\nExample of usage for 'gauss' IS-retrieving-method: python Integration_Analysis.py (--host 172.25.39.57) (--user readonly) (--pw readonlypswd) (--port 3306) --dbDataset "sequence_mld01.fu18m,sequence_mld02.fu18m" (--query_steps 5000000) (--reference_genome hg19) --columns sample,tissue,treatment (--columnsToGroup sample) --IS_method gauss (--strand_aspecific) --interaction_limit 4 --alpha 0.3 (--collision (--set_radius 4)) (--tsv) (--seqTracker)
\nExample of usage for 'dynamic' IS-retrieving-method: python Integration_Analysis.py (--host 172.25.39.57) (--user readonly) (--pw readonlypswd) (--port 3306) --dbDataset "sequence_mld01.fu18m,sequence_mld02.fu18m" (--query_steps 5000000) --reference_genome hg19 --columns sample,tissue,treatment (--columnsToGroup sample) --IS_method dynamic --interaction_limit_min 2 --interaction_limit_max 5 --sigma_paths "1,1.5,2,2.5,3" --N_simulations_per_solution 10 --in_parallel 5 (--collision (--set_radius 4)) (--tsv) (--seqTracker)
    ...
    if (strand_specific_choice == False):
        ...
        if (IS_method == "classic"):
            for covered_base in list_of_Covered_Bases[1:]:
                ...
                if ((dist == "undef") or (dist > bp_rule) or (current_covered_bases_ensemble.spanned_bases == (bp_rule + 1)) or ((current_covered_bases_ensemble.spanned_bases + dist)>(bp_rule + 1))):
