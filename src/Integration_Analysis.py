...
###Parsing Arguments############################################################################################################################################################
usage_example = '''\n\nExamples of usage for 'classic' IS-retrieving-method: python Integration_Analysis.py (--host 172.25.39.57) (--user readonly) (--pw readonlypswd) (--port 3306) --dbDataset "sequence_mld01.fu18m,sequence_mld02.fu18m" (--query_steps 1000000) (--rowthreshold 10000000) (--reference_genome hg19) --columns sample,tissue,treatment (--columnsToGroup sample) --IS_method classic (--bushman_bp_rule 2) (--strand_specific) (--collision (--set_radius 4)) (--tsv)
...
parser.add_argument('--bushman_bp_rule', dest="bushman_bp_rule", help="Minimum number of empty base-pairs between reads belonging to different cluster (also called Covered Bases Ensembles). If you chose 'classic' method to retrieve IS, this number also set the maximum dimension allowed for a Covered Bases Ensemble (n+1 bases).\nDefault option is '2', i.e. 'minimum 2 empty-bp between independent ensembles, an ensemble can span at most 3bp'. Conversely, if you chose 'gauss' method, it will be automatically set equal to interaction_limit (overriding your setting) and no limit of dimension will be set for ensembles construction.", action="store", default=2, required=False, type=int)
...
parser.add_argument('--collision', dest="collision", help="If called, collisions will be performed for each dataset with all the others.\nCollision radius is set by default equal to bushman_bp_rule+2 if --IS_method classic and fixed to 4 if --IS_method gauss, however you can override it through --set_radius option.", action="store_true", default=False, required=False)
    ...
    if (check == True):
        ...
        if (delta == None):
            ...
            if (IS_method == "classic"):
                delta = bushman_bp_rule + 2 #Set Defaults, as SciencePaper 
        else:
